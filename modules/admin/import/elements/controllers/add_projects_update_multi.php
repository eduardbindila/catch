...
while(! feof($f_pointer)){
	...
	$lighting = $project[7];
	$specifyer = $project[8];
	$quote_status = $project[10];
	$quote_value = $project[11];
	$rev_value = $project[12];
	$winning = $project[12];
	...
	$quote_price = is_numeric($quote_value) ? $quote_value : $rev_value;
...
$product_id = preg_replace("/[^a-zA-Z 0-9]+/", "",  trim($project[0]));

	//$initial_price = trim($product[1]);
	
	if($product_id) {
		$newWhen = "WHEN id LIKE '".$product_id."' THEN '".$quote_price."' ";
...
$query = "UPDATE `quotes` SET `quote_price`= CASE ".$when." END WHERE `id` IN (".$ids.");";

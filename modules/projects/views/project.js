    ...
    quoteList.forEach(function(val, index) {
...

            {
                ...
                action: function(e, dt, node, config) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            'quote_id': val['id'],
                            'hide_discount': hideDiscountDetails
                        }
                    }).success(function(json) {
                        $('.updateError').addClass('hidden');
                        location.reload();
        ...
        buttonsArray = [{
                ...
                action: function(e, dt, node, config) {
                    ...
                    console.log(productsForDeliveryDate);

                    // Activează modalul
                            $('#getDeliveryStatus-modal').modal('show');

                    $.ajax({
                        url: 'http://localhost:3030/getPromiseDate', // URL-ul serverului Node.js
                        type: 'POST',
                        ...
                        success: function(response) {
                            ...
                            // Golim conținutul anterior din timeline
                            var timeline = document.getElementById('timeline');
                            timeline.innerHTML = ''; // Sau $('#timeline').empty();

                            // Verificăm dacă răspunsul este un array
                            if (Array.isArray(response)) {
                                response.forEach(function(element) {
                                    // Creăm un nou element li pentru fiecare produs
                                    var listItem = document.createElement('li');
                                    listItem.textContent = `Product: ${element.id}, Quantity: ${element.quantity}, Promise Date: ${element.promiseDate}`;

                                    // Adăugăm elementul li la timeline
                                    timeline.appendChild(listItem);
                                });
                            } else {
                                // Dacă răspunsul nu este un array, afișăm mesajul ca text simplu
                                var listItem = document.createElement('li');
                                listItem.textContent = `Message: ${response}`;
                                timeline.appendChild(listItem);
                            }

                            // Scroll la finalul timeline-ului pentru a afișa cele mai recente informații
                            timeline.scrollTop = timeline.scrollHeight;
                        },
                        error: function(error) {
                            console.error('Error:', error);
                            // Afișăm un mesaj de eroare în modal
                            var timeline = document.getElementById('timeline');
                            var listItem = document.createElement('li');
                ...
                customize: function(doc) {
                    ...
                    for (var row = 1; row < lastRow; row++) {
                        ...
                        if (imageBase !== 'null') {
                            doc.content[1].table.body[row][1] = {
                                image: imageBase,
                                fit: [50, 50],
                                align: 'center',
                                valign: 'center'
                    ...
                    if (extraDiscount) {
                        ...
                        extraDiscountArray = extraDiscount ? [{
                            text: 'Extra Discount (' + parseFloat(quoteList[index]['extra_discount']) + '%):',
                            style: 'bold'
                        }, '-' + discountValue.toFixed(2)] : ['', ''];
                        priceBeforeArray = [{
                            text: 'Total Price:',
                            style: 'bold'
                        }, priceBeforeExtraDiscount];
                    }
                            ...
                            columns: [{
                                ...
                                table: {
                                    body: [
                                        priceBeforeArray,
                                        extraDiscountArray, [{
                                            text: 'Final Offer Price:',
                                            style: 'offerPrice'
                                        }, {
                                            text: priceAfterExtraDiscount,
                                            style: 'offerPrice'
                                        }]
                                    ]
                                },
                    ...
                    doc.content[3] = [{
                            ol: [
                                'The prices are in EUR with Bucharest DDU Delivery',                                             'Prices are without VAT and Green Tax',                                             'Delivery time / confirmed on firm order',                                             'Payment method and term: under contract',                                             'Offer valid 30 days'
                            ],
                            style: 'list'
                        }, {
